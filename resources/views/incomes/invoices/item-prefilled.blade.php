<tr id="item-row-{{ $item_row }}">
    ...
    <td {!! $errors->has('item.' . $item_row . '.name') ? 'class="has-error"' : ''  !!}>
        ...
        <select name="item[{{$item_row}}][name]" class="form-control update_drop_down_item" id="item-name-{{ $item_row }}">
            <option value="">{{trans('general.form.enter', ['field' => trans_choice('invoices.item_name', 1)])}}</option>
            @foreach($items as $one_item)
                <option @if(!empty($item) && $one_item->item_id == $item->item_id) selected="selected" @endif data-id="{{$one_item->id}}" data-company_id="{{$one_item->company_id}}" data-sale_price="{{$one_item->sale_price}}" data-quantity="{{$one_item->quantity}}" data-tax_id="{{$one_item->tax_id}}" data-item_id="{{$one_item->item_id}}" data-value="{{json_encode($one_item->toArray())}}" value="{{$one_item->id}}">{{$one_item->name}}</option>
            @endforeach
        </select>
